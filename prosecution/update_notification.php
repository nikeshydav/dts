...
function delete_notification($id) {
    ...
    $sql = "update notifications set notify_status='1', snooze='".date('Y-m-d')."' where fid='$id'";
    //exit();
    mysql_query($sql);
...
<html>
    ...
    if ($_GET['action'] == 'edit') {
        ...
        //$res=mysql_query($sql);
        $getClients = mysql_query("select a.*,g.app_id,g.status_name from application a,getappstatus g where a.id='$ides' and g.app_id='$ides' group by a.id");
        if ($getClients === FALSE) {
