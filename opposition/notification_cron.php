...
echo $sql = "Select * from ( SELECT a.app_id, a.status,DATEDIFF( now(), STR_TO_DATE( a.status_effective_on_date, '%Y-%m-%d')  ) as ef_day,  s.id as child_id, s.radio, s.time_to_execute FROM `application_status` a, `status` s where a.status=s.parent_id and  a.status_effective_on_date!='') as tmp where ef_day>=time_to_execute";
$res = mysql_query($sql);
$i = $j = $l = 0;
...
while ($row = mysql_fetch_array($res)) {
    ...
    echo $notify = "select count(*) as tot, DATE(toe) as toe from notifications where app_id='$id' and status_id='$child_id'";
