...
if (basename($_SERVER['SCRIPT_NAME']) == 'update.php') {
    ...
    <tr> <td> E-MAIL :</td> <td>
            <?php
            $ids = $obj->id;
            $sql = "SELECT u.id,uee.auto_id,uee.email FROM user u, user_entity_email uee WHERE u.id=uee.user_id and u.id = '" . $ids . "' order by id DESC ";
            ...
            while ($result = mysql_fetch_array($show)) {
                ...
                ?>
                <p>
                    <label><input type="text" name="email<?php echo $i; ?>" id="email<?php echo $i; ?>" value="<?php echo $email ?>" />
                        <input type="hidden" name="email_hidden<?php echo $i; ?>" id="email_hidden<?php echo $i; ?>" value="<?php echo $auto_id ?>">                        
                    </label>
...
<script>
    ...
    $('.removeEmail').live('click', function() {
        ...
        $(this).parent().remove();
        $input  = $(this).parent().children().children().val();
        $('#removeIds').append('<input type="hidden" name="removeEml[]" value="'+ $input +'" />');
