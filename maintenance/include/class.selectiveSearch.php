...
class selectivesearch {
    ...
    function select_search($download=FALSE) {
        ...
        $status_recordals_select = $post['status_recordals_select'];
        $condition = '';
        $select_post = '';
        $download_search = '';
        if ($alg_file_no != '') {
        ...
        if ($status_recordals_select != '' && $status_recordals_select != 'select') {
            $condition .= "and a.id in (select sr.status_recordals_app_id from status_recordals sr where sr.status_recordals like '%" . $status_recordals_select . "%')";
            $_SESSION['status_recordals_select'] = $status_recordals_select;
            $select_post .= ' Status Recordals: ' . $status_recordals_select . ';';
        }
        ...
        if ($post['application_number_chk']=='yes') {
            $c .= ($_POST) ? '<td><b>Registration No.</b></td>' : "Registration No.\t";
        ...
        if ($post['class_select_chk']=='yes') {
            $c .= ($_POST) ? '<td><b>Class(es)</b></td>' : "Class(es)\t";
            $_SESSION['class_select_chk']='yes';
        }
        if ($post['filling_select_chk']=='yes') {
            $c .= ($_POST) ? '<td><b>Filing Date</b></td>' : "Filing Date\t";
        ...
        if ($post['status_select_chk']=='yes') {
            $c .= ($_POST) ? '<td><b>Status Maintenance</b></td>' : "Status Maintenance\t";
            $_SESSION['status_select_chk']='yes';
        }
        if ($post['status_recordals_chk'] == 'yes') {
            $c .= ($_POST) ? '<td><b>Status Recordals</b></td>' : "Status Recordals\t";
            $_SESSION['status_recordals_chk'] = 'yes';
        }
        $c .= ($_POST) ? '</tr>' : "\n";
         ...
         */
        $sql = "select *, u.name AS client,  u.name AS applicant, ys.all_alg_data  AS algno, aft.all_alg_file_data  AS algflno  from application a, getappstatus g, `user` u, `user` us , year_serialno ys, alg_file_type aft where a.id>1 and g.app_id=a.id  and u.id = a.client and  us.id = a.applicant and ys.alg_app_id = a.id and aft. alg_file_app_id = a.id $condition";
        ...
        if ($nums >= 1) {
            ...
            while ($get = mysql_fetch_array($res)) {
                ...
                if ($post['status_recordals_chk']) {
                    $c .= ($_POST) ? '<td>' . $status_recordals . '</td>' : "$status_recordals\t";
                }
