...
class docket {
    ...
    function add_client_lawfirm($lawfirm, $lawfirm_code, $client_email, $client_address) {
        ...
        mysql_query($sql);
        $id = mysql_insert_id();
        $sql1 = "insert into log (user_id,action,comment) values('" . $_SESSION['id'] . "','apply_new','user_id=$id, email_id=$email')";
        mysql_query($sql1);
        $sid = mt_rand(1, 1000000);
        if (!headers_sent()) {
...
//exit();
        ...
        $sql3 = "insert into alg_file_type (alg_file_app_id, alg_file_type,alg_forum,alg_apln,alg_class,alg_mark,all_alg_file_data)
            values('$id','" . $_POST['alg_file_tyle'] . "','" . $_POST['alg_forum'] . "',' " . $_POST['alg_apln'] . "',
            '" . $classes . "','" . addslashes($_POST['alg_mark']) . "','$all_alg_file_data')";
...
<a  onclick=" return confirm(\'do you really want to delete?\');"  href="update_history_status.php?action=delete&id=' . $id . '&?sid=' . $sid . '">
    ...
    function update_user($id, $name, $name_code, $address) {
        ...
        for ($b = 1; $b <= 30; $b++) {
            //if (isset($_POST['email' . $b])) {
            $email_add = $_POST['email' . $b];
            if ($email_add != '') {
                $sql2 = "insert into user_entity_email (user_id,email) values('" . $id . "','$email_add')";
                mysql_query($sql2);
            }
