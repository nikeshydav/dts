        ...
        <div class="content1">
            <div class="welcome"><h3>Welcome <?php echo $_SESSION['username'] ?>!</h3></div>
             <table  width="100%">
                ...
                $s = "select a.id, a.fid, a.alert, a.alert_name, a.toa, ys.all_alg_data, ft.all_alg_file_data from alert a, application ap, alg_file_type ft, year_serialno ys where " . $date . " and  a.toa!='0000-00-00' and a.status=0 and a.del='0000-00-00' and ap.id=a.fid and ap.id=ft.alg_file_app_id and ap.id=ys.alg_app_id";
                $res = mysql_query($s);
                while ($row = mysql_fetch_array($res)) {
                    echo '<tr><td><b>' . $i++ . '</b></td><td>' . $row['alert'] . '</td><td>' . $row['all_alg_file_data'] . '</td><td>' . $row['all_alg_data'] . '</td><td>' . $row['toa'] . '</td><td>' . $row['alert_name'] . '</td>';
